...
// set antinode for simbol at position pos 
void setAntinode (Map& map, Position pos, Map& antinodes){
	...
	for (int i = pos.x; i < MAP_L; i++){
		for (int j = initJ ; j < MAP_H; j++){
			if (map[i][j] == simb){
				Position posSimb {i,j};
				Position diff = pos - posSimb;
				...
				// set antinodes
				if (p1.isValid() && map[p1.x][p1.y]!=simb && antinodes[p1.x][p1.y]=='.') antinodes[p1.x][p1.y] = '#';
				if (p2.isValid() && map[p2.x][p2.y]!=simb && antinodes[p2.x][p2.y]=='.') antinodes[p2.x][p2.y] = '#';
			}
		}
		initJ = 0; 
	}
...
unsigned long countAntinode (Map& map, Map& antinodes){
	...
	for (int i = 0; i < MAP_L; i++){
		for (int y = 0; y < MAP_H; y++){
			...
			if (simb != '.' && simb != '#'){
				Position pos {i,y}; 	// pos of simb 
				setAntinode (map, pos, antinodes);
			}
